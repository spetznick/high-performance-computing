...
int timer_c = 0;  /* is timer running? */
int timer_e = 0;  /* is timer running? */
int timer_g = 0;  /* is timer running? */
int timer_i = 0;  /* is timer running? */
double wtime_comp = 0;
double wtime_ex_neighbours = 0;
double wtime_global_comm = 0;
double wtime_idle = 0;
...
void print_alltimers() {
    if (timer_on) {
        stop_timer();
        printf("(%i) %14.6f s, %14.6f, %14.6f, %14.6f, %14.6f\n", proc_rank,
               wtime, wtime_comp, wtime_ex_neighbours, wtime_global_comm,
               wtime_idle);
        resume_timer();
    } else
        printf("(%i) Elapsed Wtime: %14.6f s (%5.1f%% CPU)\n", proc_rank, wtime,
...
void add_to_timer(char c) {
    switch (c) {
        case 'c':
            if (!timer_c) {
                timediff_c = MPI_Wtime();
                timer_c = 1;
            } else {
                timediff_c = MPI_Wtime() - timediff_c;
                wtime_comp += timediff_c;
                timer_c = 0;
                timediff_c = 0;
            }
            break;
        case 'e':
            if (!timer_e) {
                timediff_e = MPI_Wtime();
                timer_e = 1;
            } else {
                timediff_e = MPI_Wtime() - timediff_e;
                timer_e = 0;
                wtime_ex_neighbours += timediff_e;
                timediff_e = 0;
            }
            break;
        case 'g':
            if (!timer_g) {
                timediff_g = MPI_Wtime();
                timer_g = 1;
            } else {
                timediff_g = MPI_Wtime() - timediff_g;
                timer_g = 0;
                wtime_global_comm += timediff_g;
                timediff_g = 0;
            }
            break;
        case 'i':
            if (!timer_i) {
                timediff_i = MPI_Wtime();
                timer_i = 1;
            } else {
                timediff_i = MPI_Wtime() - timediff_i;
                timer_i = 0;
                wtime_idle += timediff_i;
                timediff_i = 0;
            }
            break;
        default:
...
void Setup_Proc_Grid() {
    ...
    if (proc_rank == 0) {
        ...
        if (N_nodes != P)
            Debug(
                "My_MPI_Init : Mismatch of number of processes in mapping "
                "inputfile",
                1);
    } else
        N_nodes = P;
    ...
    if (proc_rank == 0) {
        for (i = 0; i < N_nodes; i++) fscanf(f, "%i\n", &index[i]);
    }
    ...
    if (proc_rank == 0) {
        ...
        for (i = 0; i < N_edges; i++) fscanf(f, "%i\n", &edges[i]);
    ...
    reorder = 1;
    MPI_Graph_create(MPI_COMM_WORLD, N_nodes, index, edges, reorder,
                     &grid_comm);

    /* Retrieve new rank of this process */
    MPI_Comm_rank(grid_comm, &proc_rank);

    if (N_edges > 0) free(edges);
    free(index);
}
...
void Setup_Grid() {
    ...
    for (i = 0; i < N_vert; i++) A[i].Ncol = 0;

    /* Read all values */
    for (i = 0; i < N_vert; i++) {
        fscanf(f, "%i", &v);
        fscanf(f, "%lf %lf %i %lf\n", &vert[v].x, &vert[v].y, &vert[v].type,
               &phi[v]);
    }
...
void Add_To_Matrix(int i, int j, double a) {
    ...
    while ((k < A[i].Ncol) && (A[i].col[k] != j)) k++;
    if (k < A[i].Ncol)
        A[i].val[k] += a;
    else {
        if (A[i].Ncol >= MAXCOL) Debug("Add_To_Matrix : MAXCOL exceeded", 1);
        A[i].val[A[i].Ncol] = a;
        A[i].col[A[i].Ncol] = j;
        A[i].Ncol++;
    }
...
void Build_ElMatrix(Element el) {
    ...
    if (det == 0.0) Debug("One of the elements has a zero surface", 1);
    ...
    for (i = 0; i < 3; i++)
        if (!((vert[el[i]].type & TYPE_GHOST) |
            ...
            for (j = 0; j < 3; j++) Add_To_Matrix(el[i], el[j], s[i][j]);
}
...
void Setup_MPI_Datatypes(FILE *f) {
    ...
    for (i = 0; i < N_vert; i++) blocklens[i] = 1;
    ...
    for (i = 0; i < N_neighb; i++) {
        ...
        while (s == 1) {
            ...
            if ((s == 1) && !(vert[indices[count]].type & TYPE_SOURCE)) count++;
        }
        fscanf(f, "\n");
...
void Exchange_Borders(double *vect) {
    ...
    for (int i = 0; i < N_neighb; i++) {
        MPI_Sendrecv(&vect[0], 1, send_type[i], proc_neighb[i], 0, &vect[0], 1,
                     recv_type[i], proc_neighb[i], 0, grid_comm, &status);
    }
    add_to_timer('e');

    // Please finsih this part to realize the purpose of data communication
    // among neighboring processors. (Tip: the function "MPI_Sendrecv" needs to
    // be used here.)
}
...
void Solve() {
    ...
    for (i = 0; i < N_vert; i++) {
        r[i] = 0.0;
        for (j = 0; j < A[i].Ncol; j++) r[i] -= A[i].val[j] * phi[A[i].col[j]];
    }
    ...
    while ((count < max_iter) && (r1 > precision_goal)) {
        ...
        sub = 0.0;
        for (i = 0; i < N_vert; i++)
            if (!(vert[i].type & TYPE_GHOST)) sub += r[i] * r[i];
        ...
        if (count == 0) {
            /* p = r */
            for (i = 0; i < N_vert; i++) p[i] = r[i];
        } else {
            b = r1 / r2;

            /* p = r + b*p */
            for (i = 0; i < N_vert; i++) p[i] = r[i] + b * p[i];
        }
        add_to_timer('c');  // off
        Exchange_Borders(p);
        add_to_timer('c');  // on
        ...
        sub = 0.0;
        for (i = 0; i < N_vert; i++)
            if (!(vert[i].type & TYPE_GHOST)) sub += p[i] * q[i];
        ...
        /* x = x + a*p */
        for (i = 0; i < N_vert; i++) phi[i] += a * p[i];

        /* r = r - a*q */
        for (i = 0; i < N_vert; i++) r[i] -= a * q[i];
    ...
    if (proc_rank == 0) printf("Number of iterations : %i\n", count);
}
