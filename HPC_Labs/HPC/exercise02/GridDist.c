        ...
        for (i = 0; i < N_sources; i++) {
            ...
            source2[i].value = source_val[i]; /****/
                                              /*
                                                 ...
                                                 source_y,                                   (double) x /
                                                 (gridsize[X_DIR] - 1),                                   (double)
                                                 y /                                   (gridsize[Y_DIR] - 1), x,
                                                 y, source[i]);
                                              */
        }
    }
...
void Write_Datafiles() {
    ...
    for (py = 0; py < P_grid[Y_DIR]; py++)
        for (px = 0; px < P_grid[X_DIR]; px++) {
            ...
            if (py != 0) top = 1;
            if (py != P_grid[Y_DIR] - 1) bottom = 1;
            if (px != 0) left = 1;
            if (px != P_grid[X_DIR] - 1) right = 1;
            if (top && left) start = -1;
            if (bottom && right) end = -1;
            x_dim += left + right;
            y_dim += top + bottom;
            x_off -= left;
            y_off -= top;
            ...
            if (bottom && right) N_vert--;
            fprintf(f, "N_vert: %i\n", N_vert);
            fprintf(f, "id x y type\n");
            for (y = 0; y < y_dim; y++)
                for (x = ((y == 0) ? -start : 0);
                     x < x_dim + ((y == y_dim - 1) ? end : 0); x++) {
                    t = 0;
                    if (((x == 0) && left) || ((y == 0) && top) ||
                        ((x == x_dim - 1) && right) ||
                        ((y == y_dim - 1) && bottom))
                    ...
                    if ((x + x_off == 0) ||
                        (x + x_off == gridsize[X_DIR] - 1) ||
                        (y + y_off == 0) ||
                        (y + y_off == gridsize[Y_DIR] - 1)) {
                        t |= TYPE_SOURCE;
                        s_val = 0;
                    }
                    ...
                    if (do_adapt)
                        fprintf(
                            f, "%i %20.15e %20.15e %i ", y * x_dim + x + start,
                            grid[x + x_off + (y + y_off) * gridsize[X_DIR] + 1]
                                .xpos,
                            grid[x + x_off + (y + y_off) * gridsize[X_DIR] + 1]
                                .ypos,
                            t);
                    else
                        fprintf(f, "%i %20.15e %20.15e %i ",
                                y * x_dim + x + start,
            ...
            fprintf(f, "Neighbours: %i\n",
                    top + bottom + left + right + ((top && right) ? 1 : 0) +
                        ((bottom && left) ? 1 : 0));
            if (top) {
...
void Write_GraphMap() {
    ...
    for (py = 0; py < P_grid[Y_DIR]; py++)
        for (px = 0; px < P_grid[X_DIR]; px++) {
            if (py > 0) n++;
            if (py < P_grid[Y_DIR] - 1) n++;
            if (px > 0) n++;
            if (px < P_grid[X_DIR] - 1) n++;
            if ((py > 0) && (px < P_grid[X_DIR] - 1)) n++;
            if ((px > 0) && (py < P_grid[Y_DIR] - 1)) n++;
            fprintf(f, "%i\n", n);
        }
