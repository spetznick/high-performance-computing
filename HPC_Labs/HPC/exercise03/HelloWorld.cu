...
// Simple starting example for CUDA program : this only works on arch 2 or
// higher Cong Xiao and Senlei Wang, Modified on Sep 2018
...
__global__ void HelloworldOnGPU(void) {
    ...
    printf(
        "Hello World, I am thread %d in block with index %d, my thread index "
        "is %d \n",
...
int main(void) {
    ...
    cudaDeviceSynchronize();  // without using synchronization, output won't be
                              // shown
